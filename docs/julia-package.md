...
`LyapunovSpectrumResult` carries the ordered `exponents`, a `convergence` matrix of the running finite-time estimates (one row per accumulated interval, one column per exponent), the `estimation_status`, and `total_time`. Set `k` to track only the leading exponents. The exponents sum to the mean log volume-change rate (`log|det J|` for maps, the Jacobian trace for flows), and a bounded non-equilibrium flow always shows one numerically zero exponent — two built-in sanity checks.
