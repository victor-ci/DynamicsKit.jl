...
| Transverse-exponent field | Analytic radial-phase oscillator with a known closed-form transverse exponent; Vilnius variational field checked on the CPU seam and against cache-hook semantics. This covers estimator correctness; it does not yet reproduce a quantified spurious-classification-rate reduction across a full parameter grid — track that as a follow-up validation script rather than a closed item. |
| Robust-chaos region certification | Integration tests on an analytic cat-map fixture (single-region and two-parameter-region certification), source-result reuse across the Lyapunov/atlas layers (including malformed/mismatched-input rejection), and serializer round-trips (including empty/nonempty stable-evidence payloads). This covers the composition/aggregation logic and plain-data contracts; it does not yet include a designed embedded-periodic-strip exclusion fixture, a budget-monotonicity gate, or the physical buck-converter (C = 2 µF vs 0.1 µF) grounding case — track those as follow-up validation scripts rather than closed items. |
