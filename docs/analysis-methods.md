...
exponents sum to the mean log volume-change rate (`log|det J|` for maps; the trace of
the Jacobian for flows), and a bounded, non-equilibrium flow always has one numerically
zero exponent along the trajectory direction. `plot_lyapunov_spectrum(result)` shows the
convergence of each exponent against the accumulation horizon.
