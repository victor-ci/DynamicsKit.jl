...
const OUTPUT_DIR = joinpath(@__DIR__, "..", "var", "output", "colpitts_exponential_voltage_cascade")
mkpath(OUTPUT_DIR)

const V_MIN = 1.25
const V_MAX = 1.97
const BF_STEPS = parse(Int, get(ENV, "COLPITTS_EXP_V_BF_STEPS", "80"))
const BF_ITER = parse(Int, get(ENV, "COLPITTS_EXP_V_BF_ITER", "220"))
const BF_TRANSIENT = parse(Int, get(ENV, "COLPITTS_EXP_V_BF_TRANSIENT", "160"))
const SEARCH_MIN = [2.0, -0.82]
const SEARCH_MAX = [2.8, -0.70]
const LINKED_PARAM_INDICES = [5]
...
function run_branch(sys, case)
    params = [40e-9, 40e-9, 265.0, case.v0, case.v0]
    continuation_branch(
        sys,
        ContinuationConfig(
            p_min=case.pmin,
...
bf_path = joinpath(OUTPUT_DIR, "colpitts_exponential_v_bruteforce.png")
overlay_path = joinpath(OUTPUT_DIR, "colpitts_exponential_v_overlay.png")
